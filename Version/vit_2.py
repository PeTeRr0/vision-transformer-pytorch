...
class ViT(nn.Module):
    def __init__(self, img_size, patch_size, num_classes, dim, depth, heads, mlp_dim, pool = 'cls', channels = 3, dim_heads = 64, dropout = 0.):
        super().__init__()
        img_height, img_width = pair(img_size)
        patch_height, patch_width = pair(patch_size)

        assert img_height % patch_height == 0 and img_width % patch_width == 0, "img dim must be divisible by the patch size"
